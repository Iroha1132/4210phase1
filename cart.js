document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('https://ierg4210.eastasia.cloudapp.azure.com/csrf-token', { credentials: 'include' })
        ...
        .then(data => {
            paymentCsrfToken = data.csrfToken;
            console.log('Payment CSRF token:', paymentCsrfToken);
        })
        .catch(err => console.error('CSRF fetch error for payment:', err));
            ...
            fetch('https://ierg4210.eastasia.cloudapp.azure.com/validate-order', {
                ...
                .catch(err => {
                    ...
                    fetch('https://ierg4210.eastasia.cloudapp.azure.com/csrf-token', { credentials: 'include' })
                        ...
                        .then(data => {
                            paymentCsrfToken = data.csrfToken;
                            console.log('Refreshed Payment CSRF token:', paymentCsrfToken);
                        })
    ...
    document.addEventListener('change', (event) => {
        if (event.target.tagName === 'INPUT' && event.target.type === 'number') {
            ...
            if (newQuantity <= 0) {
                cart Rik = cart.filter(item => item.pid !== pid);
            } else {
